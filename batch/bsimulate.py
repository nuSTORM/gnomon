...
time python simulate.py --name %(db_name)s --vertex 1000 -1000 0 -g %(db_name)s --events %(number_of_events)d %(flags)s --run %(run)d --polarity %(polarity)s
""" % {'db_name' : db_name, 'number_of_events' : number_of_events, 'run' : run, 'flags':flags, 'server_url':server, 'polarity' : polarity}
        ...
        hours = float(number_of_events) / 1000.0 * 2
