...
class MyPrimaryGeneratorAction(G4VUserPrimaryGeneratorAction):
  ...
  def GeneratePrimaries(self, event):
    self.particleGun.SetParticleByName("mu-")

    self.particleGun.SetParticleEnergy(500.*MeV)
    self.particleGun.SetParticlePosition(G4ThreeVector(10,0.,0.)*cm) 
    self.particleGun.SetParticleMomentumDirection(G4ThreeVector(1.,0.,0.))  
    #self.particleGun.SetParticleMomentumDirection(G4ThreeVector(dx, 0., 1.))
    self.particleGun.GeneratePrimaryVertex(event)

    pp = G4PrimaryParticle()
    pp.SetPDGcode(211)
    pp.SetMomentum(-1000, 0, 0)
...
# ------------------------------------------------------------------
# normal way for constructing user physics list
pgPGA= MyPrimaryGeneratorAction() # g4py.ParticleGun.ParticleGunAction()
...
#pg.SetParticleEnergy(500.*MeV)
#pg.SetParticlePosition(G4ThreeVector(10,0.,0.)*cm)
...
#absorber material selection row=3
    ...
    self.absorbermaterialVar.set("Iron")
    ra1 = { }
    pos=1
    for i in ("Aluminium", "Iron"):
...
#number of event row=10
    ...
    self.eventVar.set(3)
    event = Scale(self,  orient=HORIZONTAL, length=400, from_=0, to=10**5, tickinterval=10**4, resolution=1, variable=self.eventVar )
