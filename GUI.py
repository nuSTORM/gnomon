...
class VlenfApp(Frame):
    ...
    def init(self):
        ...
        self.eventVar.set(1)
        event = Scale(self, orient=HORIZONTAL, length=400, from_=0, to=10**4,
                      tickinterval=10**4, resolution=1, variable=self.eventVar)
        eventLabel.grid(row=10, column=0, sticky=W)
        event.grid(row=10, column=1, columnspan=5, sticky=W)

        startBut = Button(self, bg="orange", text="Start a run",
                          command=self.cmd_beamOn)
        ...
        self.processList = ["phot", "compt", "conv", "msc", "eIoni", "eBrem",
                            "annihil", "muIoni", "muBrems", "hIoni"]
        pos = 1
        self.processVar = {}
        for i in self.processList:
            self.processVar[i] = IntVar()
            procTab[i] = Checkbutton(self, text=i, variable=self.processVar[i],
                                     command=self.cmd_setProcess)
            if pos <= 3:
        ...
        self.cutVar.set(1.)
        cut = Scale(self, orient=HORIZONTAL, length=400, from_=0., to=10.,
                    tickinterval=5., resolution=0.005, variable=self.cutVar,
                    digits=5)
        cutLabel.grid(row=14, column=0, sticky=W)
        cut.grid(row=14, column=1, columnspan=5, sticky=W)

        setviewXYBut = Button(self, bg="red", text="X-Y",
                              command=self.cmd_setviewXY)
        setviewXYBut.grid(row=16, column=2, sticky=W)

        setviewZYBut = Button(self, bg="red", text="Z-Y",
                              command=self.cmd_setviewZY)
        setviewZYBut.grid(row=16, column=3, sticky=W)

        setviewZXBut = Button(self, bg="red", text="Z-X",
                              command=self.cmd_setviewZX)
        setviewZXBut.grid(row=16, column=4, sticky=W)

        shrinkBut = Button(self, bg="red", text="shrink",
                           command=self.cmd_shrink)
        shrinkBut.grid(row=16, column=0, sticky=W)

        expandBut = Button(self, bg="red", text="expand",
                           command=self.cmd_expand)
        ...
        comBut = Button(self, bg="orange", text="Execute",
                        command=self.cmd_g4command)
